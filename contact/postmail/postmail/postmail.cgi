...
sub send_mail {
	...
	if ($cf{conv_pm} == 2) {
		...
		$mail = Unicode::Japanese->new($mail,'utf8')->conv('jis');
	} else {
		$mail =~ s/\xef\xbd\x9e/\xe3\x80\x9c/g;
		$mail =~ s/\xef\xbc\x8d/\xe2\x88\x92/g;
		$mail = $j->set(\$mail,'utf8')->jis;
	}
	...
	if ($cf{auto_res}) {
		...
		if ($cf{conv_pm} == 2) {
			$reply = Unicode::Japanese->new($reply,'utf8')->conv('jis');
		} else {
			$reply =~ s/\xef\xbd\x9e/\xe3\x80\x9c/g;
			$reply =~ s/\xef\xbc\x8d/\xe2\x88\x92/g;
			$reply = $j->set(\$reply,'utf8')->jis;
		}
	}
	...
	# コード変換
	if ($cf{conv_pm} == 2) {
		$mbody = Unicode::Japanese->new($mbody,'utf8')->conv('jis');
	} else {
		$mbody =~ s/\xef\xbd\x9e/\xe3\x80\x9c/g;
		$mbody =~ s/\xef\xbc\x8d/\xe2\x88\x92/g;
